...
//	    double uedensity = addJetUe(particles, (jetuebranch->eventUe).at(iBranch), *iProtoJet, radius);
            double uedensity, ueMdensity;
            std::tie(uedensity, ueMdensity)  = addJetUe(particles, (jetuebranch->eventUe).at(iBranch), *iProtoJet, radius);
	    currentjet->setUeDensity(jetuebranch->name, uedensity);
            currentjet->setUeMDensity(jetuebranch->name, ueMdensity);
	  }
	}
	// Clean up particles
...
std::pair<float, float> StJetMaker2012::addJetUe(StProtoJet::FourVecList particles, StUeOffAxisConesEvent *ueEvent, const StProtoJet &jet, double radius)
{
  ...
  double jVecAreaPx = jet.vecAreaPx();
  double jVecAreaPy = jet.vecAreaPy();
  double jVecAreaPz = jet.vecAreaPz();
  double jVecAreaE = jet.vecAreaE();

  const int Ncone = 2;
  StUeOffAxisCones *cones[Ncone];
  const double PI = 3.14159;
 
  TLorentzVector coneVec0, coneVec1; 
  double pt[Ncone];
  int number[Ncone];
  ...
  for(size_t ii = 0; ii < particles.size(); ii++){
    ...
    TLorentzVector coneParticle;
    coneParticle.SetPxPyPzE(vect->px(), vect->py(), vect->pz(), vect->e());
    if(dR < radius){
      pt[0] += p_pt;
      number[0]++;
      coneVec0 += coneParticle;
      index = 0;
    }else{
      dR = DeltaR(p_eta, p_phi, cone_eta[1], cone_phi[1]);
      if(dR < radius){
        pt[1] += p_pt;
        coneVec1 += coneParticle;
        number[1]++;
	index = 1;
      }
    }
  ...
  double cone_mass_0 = coneVec0.M();
  double cone_mass_1 = coneVec1.M();

  if(cone_mass_0 < 0.) cone_mass_0 = 0.;
  if(cone_mass_1 < 0.) cone_mass_1 = 0.;

  double Mdensity = (cone_mass_0 + cone_mass_1)/(Ncone*PI*radius*radius);
  ...
  jcones->setVecArea(jVecAreaPx,jVecAreaPy,jVecAreaPz,jVecAreaE);
  jcones->setDensity(density);
  jcones->setMDensity(Mdensity);
  ...
  return std::make_pair(density, Mdensity);
//  return density;
}
