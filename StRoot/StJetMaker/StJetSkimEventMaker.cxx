...
Int_t StJetSkimEventMaker::Make()
{
	...
	// VPD Stuff:
	// Start: bassam
	// declaring pile-up variables for pile-up information from the the VPD
	/*int vpdTotWHits;
        int vpdTotEHits; 
        int vpdGoodWHits;
        int vpdGoodEHits;
        double eVpdPileUpInfo[3];
        double wVpdPileUpInfo[3];
        // obtaining the highiest postive-rank vertex and its z location
        assert(muDst->primaryVertex(0));
        StMuPrimaryVertex* vertex = muDst->primaryVertex(0);
        assert(vertex);
	double vertZ = vertex->position().z();*/
	// End: bassam
	if (muDst->btofHeader()){
		...
		//muDst->btofHeader()->getVpdHitInfo(&vpdTotEHits, &vpdTotWHits, &vpdGoodEHits, &vpdGoodWHits, eVpdPileUpInfo, wVpdPileUpInfo, vertZ); 
		/*mEvent->setNVpdEGoodHits(vpdGoodEHits);
		mEvent->setNVpdWGoodHits(vpdTotWHits);
		mEvent->setEarliestVpdEHit(eVpdPileUpInfo[0]);
		mEvent->setEarliestVpdWHit(wVpdPileUpInfo[0]);
		mEvent->setClosestVpdEHit(eVpdPileUpInfo[1]);
		mEvent->setClosestVpdWHit(wVpdPileUpInfo[1]);
		mEvent->setLatestVpdEHit(eVpdPileUpInfo[2]);
		mEvent->setLatestVpdWHit(wVpdPileUpInfo[2]);*/
